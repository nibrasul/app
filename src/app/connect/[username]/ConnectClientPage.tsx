...
const getPlatformIcon = (platform: string) => {
  ...
  if (p.includes('website') || p.includes('globe') || p.includes('link')) {
    return (
      <svg width="24" height="24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" viewBox="0 0 24 24">
        <circle cx="12" cy="12" r="10" />
...
END:VCARD`;
  ...
  const getRolesText = () => {
    const list = profileData?.tags
      ?.filter((t: any) => t.type === 'role')
      .map((t: any) => t.text) || [];
    if (list.length === 0) return profileData ? '' : 'Creator • Developer';
    return list.join(' • ');
  };
  ...
  const getSocialsList = () => {
    ...
    if (list.length === 0) {
      return [
        ...
        { id: 4, platform: 'Instagram', url: '#' },
        { id: 5, platform: 'Instagram', url: '#' },
        { id: 6, platform: 'Instagram', url: '#' },
        { id: 7, platform: 'Instagram', url: '#' },
        { id: 8, platform: 'Instagram', url: '#' },
        { id: 9, platform: 'Instagram', url: '#' },
      ];
    }
    return list;
  };
